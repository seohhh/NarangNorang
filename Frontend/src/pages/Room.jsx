...
class Room extends Component {
  constructor(props) {
    ...
    this.guestJoinSession = this.guestJoinSession.bind(this);
  ...
  componentDidMount() {
    ...
    // 세션 아이디가 URL 매개변수로 전달된 경우 즉, url로 들어온 사람 -> guest
    else {
      this.setState({
  ...
  camStatusChanged() {
    this.setState(
      (prevState) => ({ videoOn: !prevState.videoOn }),
      () => {
        // After updating the state, toggle the publisher video
  ...
  micStatusChanged() {
    this.setState(
      ...
      () => {
        ...
        console.log(this.state)
        if (this.state.publisher) {
          this.state.publisher.publishAudio(this.state.audioOn);
        }
      }
    );
  }
